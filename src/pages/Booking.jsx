...
const Booking = () => {
  ...
  const razorpayKeyPromiseRef = useRef(null);

  const apiUrl = import.meta.env.VITE_API_URL || "http://localhost:5000";
  const fallbackRazorpayKey = import.meta.env.VITE_RAZORPAY_KEY || "";

  useEffect(() => {
    if (!state) return;
    razorpayKeyPromiseRef.current = fetchRazorpayKey(
      apiUrl,
      fallbackRazorpayKey
    );
    return () => {
      razorpayKeyPromiseRef.current = null;
    };
  }, [location.key, apiUrl, fallbackRazorpayKey]);
  ...
  const selectedCar =
    state.tab === "Round Trip"
      ? cars.find((c) => c.category === state.carName)
      : null;
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // 0️⃣ Key (prefetch when possible) + 1️⃣ create order — run in parallel
      const pendingKey = razorpayKeyPromiseRef.current;
      const keyPromise = pendingKey
        ? pendingKey.catch(() =>
            fetchRazorpayKey(apiUrl, fallbackRazorpayKey)
          )
        : fetchRazorpayKey(apiUrl, fallbackRazorpayKey);

      console.log("📝 Creating order for amount:", advance);
      const orderResPromise = fetch(`${apiUrl}/api/payment/create-order`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amount: Number(advance) }),
      });

      const [razorpayKey, res] = await Promise.all([keyPromise, orderResPromise]);
        ...
        <div className="bg-white p-5 rounded-xl shadow">
          ...
          <div className="text-sm space-y-2 text-gray-700">
            ...
            {state.tab === "Round Trip" && state.returnDate && (
              <p><b>Return:</b> {state.returnDate}</p>
            )}
            {state.carName && (
              <p><b>Vehicle:</b> {state.carName}</p>
            )}
          </div>
          ...
          {state.tab === "Round Trip" &&
            selectedCar &&
            roundTripDays != null && (
              <div className="mt-4 p-3 bg-slate-50 border border-slate-100 rounded-lg text-xs text-slate-700 space-y-1">
                ...
                <p>
                  ...
                  up to {ROUND_TRIP_KM_PER_DAY} km per calendar day
                </p>
                <p>
                  <span className="font-medium text-slate-800">
                    Per-km rate:
                  </span>{" "}
                  ₹{state.adminRatePerKm ?? selectedCar.rate}/km
                </p>
                <p className="text-slate-500">
                  {ROUND_TRIP_KM_PER_DAY} km × {roundTripDays}{" "}
                  {roundTripDays === 1 ? "day" : "days"} × ₹{state.adminRatePerKm ?? selectedCar.rate}/km
                </p>
                <p className="font-semibold text-slate-800">
                  Pay to Driver: ₹{state.fare - advance}
                </p>
              </div>
            )}
          ...
          {state.tab === "Rental" && (
            <div className="mt-4 p-3 bg-slate-50 border border-slate-100 rounded-lg text-xs text-slate-700 space-y-1">
              <p className="font-semibold text-slate-800">Rental fare details</p>
              <p>
                <span className="font-medium text-slate-800">
                  Rate / km:
                </span>{" "}
                ₹{state.adminRatePerKm ?? "N/A"}/km
              </p>
              {state.adminExtraKm != null && (
                <p>
                  <span className="font-medium text-slate-800">
                    Extra km:
                  </span>{" "}
                  ₹{state.adminExtraKm}/km
                </p>
              )}
              {state.adminExtraHour != null && (
                <p>
                  <span className="font-medium text-slate-800">
                    Extra hour:
                  </span>{" "}
                  ₹{state.adminExtraHour}/hr
                </p>
              )}
              <p className="font-semibold text-slate-800">
                Estimated total: ₹{state.fare}
              </p>
            </div>
          )}
        </div>
