...
const UpdateFares = () => {
  ...
  const deleteOneWayRoute = (routeKey) => {
    setFares((prev) => {
      const nextOneWayRates = { ...prev.oneWayRates };
      delete nextOneWayRates[routeKey];
      return {
        ...prev,
        oneWayRates: nextOneWayRates,
      };
    });
  };

  const removeNewRoute = (routeId) => {
    setExtraOneWayRoutes((prev) => prev.filter((route) => route.id !== routeId));
  };

  const oneWayRouteEntries = Object.entries(fares.oneWayRates).sort(([a], [b]) =>
    a.localeCompare(b)
  );
  ...
  const updateNewRouteField = (routeId, field, value) => {
    if (field === "pickupCity") {
      ensurePickupOption(value);
    }
    if (field === "dropCity") {
      ensureDropOption(value);
    }
      ...
      <div className="space-y-8">
        <section className="bg-white rounded-xl shadow p-6 overflow-x-auto">
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {oneWayRouteEntries.map(([routeKey, rates]) => {
                const [pickupCity, dropCity] = routeKey.split("-");
                return (
                  <tr key={routeKey} className="border-t hover:bg-gray-50">
                    ...
                    {vehicleTypes.map((vehicle) => {
                      const currentFare = rates?.[vehicle.key] || 0;
                      return (
                    ...
                    <td className="p-3 border text-center">
                      <button
                        type="button"
                        onClick={() => deleteOneWayRoute(routeKey)}
              ...
              {extraOneWayRoutes.map((route) => (
                <tr key={route.id} className="border-t bg-slate-50 hover:bg-slate-100">
                  ...
                  <td className="p-3 border text-center">
                    <button
                      type="button"
                      onClick={() => removeNewRoute(route.id)}
                      className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600"
                    >
                      Delete
                    </button>
