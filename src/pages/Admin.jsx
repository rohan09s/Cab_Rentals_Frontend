...
const Admin = () => {
  ...
  // Split online bookings by trip type
  const oneWayTrips = onlineBookings.filter((b) => b.tripType === "One Way");
  const roundTrips = onlineBookings.filter((b) => b.tripType === "Round Trip");
  const rentalTrips = onlineBookings.filter((b) => b.tripType === "Rental");
  ...
  const updateRemarks = async (bookingId, newRemarks) => {
    try {
      const token = sessionStorage.getItem("adminToken");
      const response = await fetch(`${apiUrl}/api/bookings/${bookingId}/remarks`, {
        method: "PATCH",
        headers: {
          Authorization: token,
      ...
      if (response.ok) {
        ...
        setEditingRemarks({ ...editingRemarks, [bookingId]: false });
      }
    } catch (err) {
      ...
      <div className="space-y-6">
        ...
        <section className="bg-white rounded-xl shadow p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div>
              <h2 className="text-xl font-semibold">Online Bookings - One Way Trip</h2>
              <p className="text-sm text-gray-500">One-way bookings created through the website payment flow.</p>
            </div>
            <span className="text-sm text-gray-600">{oneWayTrips.length} record{oneWayTrips.length !== 1 ? "s" : ""}</span>
          </div>
                ...
                {oneWayTrips.length ? (
                      ...
                      <td className="text-left">
                        {editingRemarks[b._id] ? (
                          <div className="flex gap-2">
                            <input
                              ...
                              onChange={(e) => {
                                const newBookings = bookings.map(x => 
                                  x._id === b._id ? { ...x, remarks: e.target.value } : x
                                );
                            ...
                            />
                            <button
                              onClick={() => updateRemarks(b._id, b.remarks || "")}
                          ...
                          <div
                            onClick={() => setEditingRemarks({ ...editingRemarks, [b._id]: true })}
                  ...
                  <tr>
                    <td colSpan="10" className="p-6 text-center text-gray-500">
                      No one-way trip bookings available.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <section className="bg-white rounded-xl shadow p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div>
              <h2 className="text-xl font-semibold">Online Bookings - Round Trip</h2>
              <p className="text-sm text-gray-500">Round-trip bookings created through the website payment flow.</p>
            </div>
            <span className="text-sm text-gray-600">{roundTrips.length} record{roundTrips.length !== 1 ? "s" : ""}</span>
          </div>
                ...
                {roundTrips.length ? (
                  roundTrips.map((b, i) => (
                    <tr key={b._id || i} className="border-t text-center">
                      ...
                      <td className="text-left">{b.fromCity} → {b.toCity}</td>
                      <td className="text-left">{b.date}</td>
                      <td className="text-left">{b.returnDate || "—"}</td>
                      <td className="text-left">{b.time}</td>
                      <td className="text-left">{b.car}</td>
                      <td className="text-left">₹{b.fare}</td>
                      <td className="text-left text-green-600 font-semibold">{b.status}</td>
                  ...
                  <tr>
                    <td colSpan="10" className="p-6 text-center text-gray-500">
                      No round-trip bookings available.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <section className="bg-white rounded-xl shadow p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div>
              <h2 className="text-xl font-semibold">Online Bookings - Rental Trip</h2>
              <p className="text-sm text-gray-500">Rental bookings created through the website payment flow.</p>
            </div>
            <span className="text-sm text-gray-600">{rentalTrips.length} record{rentalTrips.length !== 1 ? "s" : ""}</span>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {rentalTrips.length ? (
                  ...
                  <tr>
                    <td colSpan="10" className="p-6 text-center text-gray-500">
                      No rental trip bookings available.
                    </td>
                  </tr>
                )}
              </tbody>
                ...
                {manualBookings.length ? (
                  manualBookings.map((b, i) => (
                    <tr key={b._id || i} className="border-t text-center">
                      ...
                      <td className="text-left">{b.remarks || "—"}</td>
                      <td className="text-left text-green-600 font-semibold">{b.status}</td>
                      <td className="p-2 text-right">
                        {b._id ? (
                          <button
