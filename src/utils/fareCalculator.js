...
export const calculateFare = async (tab, car, state = {}, apiUrl = "") => {
  ...
  if (tab === "One Way") {
    ...
    const route = faresData.oneWayRates?.[key] || {};
        ...
        const from = normalizeCity(state.fromCity);
        const to = normalizeCity(state.toCity);

        const key = `${from}-${to}`;

        // Try exact match first
        let route = faresData.oneWayRates?.[key];

        // Fallback: case-insensitive lookup over keys
        if (!route && faresData.oneWayRates) {
          const lowerKey = key.toLowerCase();
          const foundKey = Object.keys(faresData.oneWayRates).find(
            (k) => k.toLowerCase() === lowerKey
          );
          if (foundKey) route = faresData.oneWayRates[foundKey];
        }
