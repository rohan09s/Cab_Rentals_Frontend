...
const PopularRoutes = ({ limit = null }) => {
  const [selected, setSelected] = useState(null);
  const displayedRoutes = limit ? routes.slice(0, limit) : routes;
    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      {limit && routes.length > limit && (
        <div className="mt-8 text-center">
          <Link
            to="/popular-routes"
            className="inline-flex items-center justify-center rounded-full bg-blue-600 text-white px-6 py-3 text-sm font-semibold hover:bg-blue-700 transition-colors"
          >
            View More
          </Link>
        </div>
