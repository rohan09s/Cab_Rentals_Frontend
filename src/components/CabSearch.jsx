...
const formatCity = (city) => city.charAt(0) + city.slice(1).toLowerCase();
const ALL_DESTINATIONS = Array.from(
  new Set(Object.values(routesMap).flat().map(formatCity))
).sort();
...
const CabSearch = () => {
  ...
  const vehicleExtras = useMemo(
    () => (vehicleType ? rentalPricing[vehicleType].extras : null),
    [vehicleType]
  );

  const dropCityOptions = useMemo(() => {
    if (tab !== "One Way") return [];

    if (!fromCity) {
      return ALL_DESTINATIONS;
    }

    return (routesMap[fromCity] || []).map(formatCity);
  }, [fromCity, tab]);
          ...
          <div className="md:col-span-3 flex gap-2 items-end">
            ...
            <div className="flex-1">
              ...
              >
                ...
                {fromCity && !PRIMARY_CITIES.includes(fromCity) && (
